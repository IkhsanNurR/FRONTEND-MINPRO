...
function AccountReducer(state = initialstate,action:any) {
    ...
    if (payload?.status === 200 && (type === "UPDATE_ACCOUNT_RESPONSE")
    || type === "ADD_ACCOUNT_RESPONSE" || type === "DELETE_ACCOUNT_RESPONSE") {
        setTimeout(() => {
            alert.notifySuccess(payload.status,payload.message)
        }, 500);
    } else if (payload?.status  !=200 && (type === "UPDATE_ACCOUNT_RESPONSE")
    || type === "ADD_ACCOUNT_RESPONSE" || type === "DELETE_ACCOUNT_RESPONSE"){
        setTimeout(() => {
