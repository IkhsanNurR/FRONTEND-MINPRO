...
function fintechReducer(state = initialstate,action:any) {
    ...
    if (payload?.status === 200 && (type === "UPDATE_FINTECH_RESPONSE")
    || type === "ADD_FINTECH_RESPONSE" || type === "DELETE_FINTECH_RESPONSE") {
        setTimeout(() => {
            alert.notifySuccess(payload.status,payload.message)
        }, 500);
    } else if (payload?.status  !=200 && (type === "UPDATE_FINTECH_RESPONSE")
    || type === "ADD_FINTECH_RESPONSE" || type === "DELETE_FINTECH_RESPONSE"){
        setTimeout(() => {
