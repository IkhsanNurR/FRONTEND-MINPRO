...
import { MyPage } from "@/components/types";
import { ToastContainer } from "react-toastify";
import { Example } from "@/helper/carousel";
import { TestimoniReview } from "@/helper/carousel";
...
const Bootcamp: MyPage = () => {
  ...
  return (
    <div className="mt-7">
      ...
      <div className="mt-4 bg-gray-50 w-full p-4 text-center">
        <label htmlFor="search" className="mr-2">
          Seacrh
        </label>
        <input type="search" className="p-1 border-2 rounded-xl w-28 md:w-32" />
        <button className="order-0  ml-2 inline-flex items-center px-4 py-2 border border-transparent rounded-xl bg-blue-500 text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 sm:order-1">
          Search
        </button>
      </div>
      <div className="mt-4 bg-white p-4">
        <div className="flex flex-wrap justify-center bg-white  rounded-2xl">
          {Card.map((card: any) => (
            <div className="w-80 mx-4 shadow-xl rounded-lg p-4 my-4">
              <img
                src={card.gambar}
              ...
              <div className="mt-2">
                ...
                <div className="text-end mt-2">
                  <button
                    onClick={() => router.push(`/bootcamp/${card.judul}`)}
                    className="order-0 px-4 py-1 border border-transparent rounded-xl bg-blue-500 text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-oranbg-orange-700 sm:order-1 hover:bg-blue-600"
                  >
                    Curriculum
                    <ArrowRight />
                  </button>
      ...
      <div className="mt-16">
        <h1>Testimoni</h1>
        <div className="bg-white  mt-1 p-4 mb-5">
          <TestimoniReview />
        </div>
      </div>
    </div>
